...
const Checkout = () => {
  ...
  return (
    <>
      <div className='relative container mx-auto p-5 z-0'>
        <div className='w-full mt-16 h-screen text-center z-0'>
          <div className='relative grid grid-cols-1 md:grid-cols-2 gap-5'>
            ...
            <div className='border rounded'>
              <h2 className='mt-2 text-center text-2xl'>Your choices</h2>
              <div className='p-5 max-h-96 md:max-h-[32rem] overflow-auto'>
                    ...
                    <div className='bg-black text-white w-full rounded shadow-md shadow-black/50' key={item.id}>
                      <div className='flex items-center gap-24 border-b border-white'>
                        <div className='flex w-full flex-shrink-0 justify-between items-center'>
                          <img className='photo' src={item.image} alt="img" width={70}/>
                          <h2>{item.name}</h2>
                          <p className='mr-2'>{item.quantity} x ${item.price}</p>
                        </div>
                        <br />
                      </div>
                    </div>
                ...
                <h2 className='text-end p-5 uppercase'>total: ${totalAmount()}</h2>
              </div>
            </div>
          </div>
        </div>
        <div className='mx-auto text-center md:-mt-52'>
          {orderId
            ? <>
